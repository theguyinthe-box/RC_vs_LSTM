...
import time
import json
from py_lstm_rossler_eval.logger import Logger  # nimmt an: hat set_training_time, set_model_size
...
class AgentNode(Node):
    def __init__(self):
        ...
        # Timer: periodic check if data should be sent
    ...
    def handle_hyperparams(self, msg):
        ...
        self.get_logger().info(f"Hyperparameter received: {params}")
    ...
    def periodic_check(self):
        ...
        if not self.sent and self.run_count < self.warmup_runs + self.eval_runs:
            ...
            # vor NEXT zurücksetzen, damit handle_hyperparams eine frische Logger-Instanz anlegt
            self._reset_for_next_config()

            # NEXT an den Edge signalisieren
            try:
                self.sweep_ctrl_pub.publish(String(data="NEXT"))
                self.get_logger().info("Signalisierte an Edge: NEXT (nächste Hyperparameterzeile)")
            except Exception as e:
                self.get_logger().error(f"Fehler beim Senden von NEXT: {e}")

            # NICHT herunterfahren – wir warten auf neue Hyperparameter
            return

    def _reset_for_next_config(self):
        # Alles zurücksetzen, damit der Agent auf neue Hyperparameter wartet
        self.logger = None
        self.received_params = False
        self.sent = False
        self.send_time = None
        self.run_count = 0
        self.training_time = None
        # Daten (X_scaled, Y_scaled, test_scaled) bleiben unverändert – genau so gewollt
    ...
    def send_data(self):
        if self.publisher.get_subscription_count() > 0:
            ...
            self.get_logger().info(f"Rössler data sent ({phase} {self.run_count + 1})")
            self.sent = True
        else:
