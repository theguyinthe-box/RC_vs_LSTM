...
class Logger:
    # Auf diese HPs matchen wir für Upsert/Overwrite
    _MATCH_KEYS = ["hidden_size", "num_layers", "sequence_length", "lr"]
    _FLOAT_KEYS = {"lr"}
    _INT_KEYS = {"hidden_size", "num_layers", "sequence_length"}
    _FLOAT_TOL = 1e-9  # Toleranz für Floats beim Gleichheitsvergleich
    ...
    # ---- Matching auf die 6 gewünschten Keys (robust normalisiert) ----
    ...
    def _find_existing_index_by_selected_hparams(self, df_old: pd.DataFrame,
        ...
        """Finde Index einer Zeile, deren hp.*-Spalten exakt den 5 ausgewählten Hyperparametern entsprechen."""
        if df_old is None or df_old.empty:
            return None

        # Sicherstellen, dass hp.*-Spalten für die Selektionskeys existieren
        for k in self._MATCH_KEYS:
            col = f"hp.{k}"
    ...
    def _upsert_best(self, df_old: pd.DataFrame, df_new_row: pd.DataFrame,
        ...
        """
        Upsert anhand der 5 Schlüssel-HPs:
        - gleiche 5 HP vorhanden → überschreibe nur, wenn neuer metric besser (kleiner)
        - ansonsten: füge neue Zeile hinzu
        """
        ...
        if idx is not None:
            # Parameterset existiert → prüfen, ob besser
            ...
            # sonst: NICHT speichern → unverändert zurück
            return df_old
        else:
            # Parameterset neu → Zeile anhängen
            if df_old.empty:
                return df_new_row
    ...
    def summary(self):
        ...
        # Sheet „Results“ (Best Average Prediction)
        row_pred = {
            **base_row_common,
        ...
        # Sheet „Roundtrip“ (Best Roundtrip)
        row_rtt = {
            **base_row_common,
