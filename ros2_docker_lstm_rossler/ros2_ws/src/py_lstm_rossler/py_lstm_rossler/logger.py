...
class Logger:
    ...
    # ---- Matching on the 4 selected keys (robust normalization) ----
    ...
    def _find_existing_index_by_selected_hparams(self, df_old: pd.DataFrame,
        ...
        """Find the index of a row whose hp.* columns exactly match the selected hyperparameters."""
    ...
    def _upsert_best(self, df_old: pd.DataFrame, df_new_row: pd.DataFrame,
        ...
        """
        Upsert based on the selected HPs:
        - If the same HP combo exists → overwrite only if the new metric is better (smaller)
