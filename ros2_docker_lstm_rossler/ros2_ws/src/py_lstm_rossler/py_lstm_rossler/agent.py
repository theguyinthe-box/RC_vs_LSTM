...
import time
import json
from py_lstm_rossler.logger import Logger  # assumes: has set_training_time, set_model_size

class AgentNode(Node):
    def __init__(self):
        super().__init__('agent_lstm_rossler_node')
        ...
        self.last_params = None  # stores the last received hyperparameters (incl. sequence_length)

        self.get_logger().info("Agent started, waiting for hyperparameters from Edge...")

        # Prepare data (unscaled → scaled)
        self.prepare_data()

        # Timer: periodically check whether to send
    ...
    # -------- ROS callbacks --------
    def handle_hyperparams(self, msg: String):
        params = json.loads(msg.data)
        self.last_params = params

        if self.logger is None:
            # Logger receives the hyperparameters incl. sequence_length
            self.logger = Logger("LSTM", params, self.get_logger())

        self.received_params = True
        self.get_logger().info(f"Hyperparameters received from Edge: {params}")

        # Optional: consistency checks / hints
        if "sequence_length" in params:
            seq_len = int(params["sequence_length"])
            if seq_len >= self.train_len:
                self.get_logger().warn(
                    f"sequence_length={seq_len} >= train_len={self.train_len}. "
                    "Edge will validate this; prediction may fail otherwise."
                )

    def handle_training_time(self, msg: String):
        try:
    ...
    def handle_model_size(self, msg: String):
        try:
            ...
            self.get_logger().error("Error parsing model size")
    ...
    def send_data(self):
        if self.publisher.get_subscription_count() > 0:
            ...
            # Send ONLY scaled data (Edge operates entirely on scaled data)
            ...
            self.get_logger().info("Rossler data sent")
            self.sent = True
        else:
            self.get_logger().info("Waiting for Edge node...")

    def handle_prediction(self, msg: Float64MultiArray):
