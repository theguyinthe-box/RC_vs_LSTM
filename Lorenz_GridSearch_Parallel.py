...
# Generate Lorenz dataset by solving the system
...
# Train the reservoir and generate predictions
def train_and_generate(params, X_train, Y_train, scaler, predict_len, test, idx):
    ...
    # Calculate mean squared errors on original and scaled data
...
# Execute hyperparameter grid search in parallel
...
# Print results and track best model
best_mse = float('inf')
...
for mse, mse_scaled, output, output_scaled, params, idx in results:
    print(f"[{idx}] MSE: {mse:.6f} | MSE_scaled: {mse_scaled:.6f} | Parameters: {params}")
    if mse < best_mse:
        best_mse = mse
...
# Detailed output of the best model after search completes
print("\n=== Best Model Summary ===")
print(f"Model Index: {best_idx}")
print(f"Hyperparameters: sr={best_params['sr']}, lr={best_params['lr']}, units={best_params['units']}, ridge={best_params['ridge']}")
print(f"Unscaled MSE (total): {best_mse:.6f}")
print(f"Scaled MSE (total): {best_mse_scaled:.6f}")

# MSE per dimension in unscaled space
...
# MSE per dimension in scaled space
...
# Visualization of prediction vs true Lorenz attractor
